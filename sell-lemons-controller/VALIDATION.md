...
- 10 direct-upgrade probe groups for v2: a runtime where all game-module require calls fail, scheduling before waiting, owned recorded identities only, pending exclusions, duplicate identity rejection, replicated-level receipts, RPC rejection, reset counters (including previously absent counters), owner changes, and timeout with late completion. This standalone one-round transport probe leaves affordability to the server and is not part of the controller.
